 ...
 function fn_emailSet(){
	    ...
	    $("#emailList option:selected").each(function () {
				...
				if($(this).val() == 'non'){ //직접입력일 경우
					 ...
					 // "disabled" 활성화 -> 화면에 표시만 되고 서버로 값 전달 X
				}
		   });
	}
...
function fn_modify_member_info(attribute){
	...
	// alert("mod_type:"+mod_type);
		...
		if(attribute=='member_pw'){
			...
			var sms_yn=frm_mod_member.sms_yn;
			...
			value_hp2=hp2.value;
			value_hp3=hp3.value;
			value_sms_yn=sms_yn.checked;
			value=value_hp1+","+value_hp2+", "+value_hp3+","+value_sms_yn;
			...
			var address=frm_mod_member.address;

			value_zipcode=zipcode.value;
			value_address=address.value;
			value=value_zipcode+","+value_address;
		}
			...
			<tbody>
				...
				<tr class="dot_line">
					...
					<td>
					 ...
					 <c:choose> 
					   <c:when test="${memberInfo.sms_yn=='true' }">
					     <input type="checkbox"  name="sms_yn" value="Y" checked /> 쇼핑몰에서 발송하는 SMS 소식을 수신합니다.
						</c:when>
						<c:otherwise>
						  <input type="checkbox"  name="sms_yn" value="N"  /> 쇼핑몰에서 발송하는 SMS 소식을 수신합니다.
						</c:otherwise>
					 </c:choose>	
				    </td>
					<td>
				...
				<tr class="dot_line">
					...
					<td>
					   <input type="text" name="email1" size=10 value="${memberInfo.email1}" /> @ <input type="text" size=10  name="email2" id="email2" value="${memberInfo.email2}" /> 
				...
				<tr class="dot_line">
					...
					<td>
					  ...
					  <br>
					  <p> 
					  	지번 주소:<br><input type="text" id="address"  name="address" size="50" value="${memberInfo.address}"><br><br>
					  </p>
					</td>
					<td>
